...
function normalizePersisted(value: unknown): PersistedSession {
    ...
    return {
        ...
        adminEmail: typeof v.adminEmail === 'string' ? v.adminEmail : defaults.adminEmail,
        runId: typeof v.runId === 'string' ? v.runId : defaults.runId,
        accessCode: typeof v.accessCode === 'string' ? v.accessCode : defaults.accessCode,
        startTime: typeof v.startTime === 'number' ? v.startTime : defaults.startTime,
    }
}
...
class SessionStore {
    ...
    setAdmin({ adminRole, adminEmail }: { adminRole: AdminRole | null; adminEmail: string }) {
    ...
    startRun({ runId, accessCode, startTime }: { runId: string; accessCode: string; startTime?: number | null }) {
        this.runId = runId ?? ''
