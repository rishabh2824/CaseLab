...
import { mapContact, normalizeHistories, type MappedContact } from './Helpers.js'
import type {
    ChatMessage,
    Contact,
    NotesPayload,
...
class RunStore {
    ...
    contacts = $derived<MappedContact[]>((this.raw?.contacts ?? []).map(mapContact))
    sharedFiles = $derived<SharedFile[]>(this.raw?.shared_files ?? [])
    serverHistories = $derived<Record<string, ChatMessage[]>>(normalizeHistories(this.raw?.histories ?? {}))
    ...
    async refresh(runId: string): Promise<void> {
        try {
            ...
            if (err instanceof ApiError && err.status === 404) {
                ...
                this.loadError = (err instanceof Error && err.message) || 'Failed to load the simulation.'
            }
        }
    }
    ...
    async #runSend(personaId: string, message: string): Promise<void> {
        ...
        try {
            ...
            if (err instanceof Error && err.message === 'This conversation has ended.') {
                const target = this.raw?.contacts?.find((c) => c.id === personaId)
                if (target) {
