...
export const slugifyFileName = (value: unknown): string => slugify(value) || 'case-lab-chat-export'
...
const writeSection = (doc: jsPDF, title: string, bodyLines: string[]): void => {
    ...
    for (const rawLine of bodyLines) {
        const wrapped: string[] = rawLine ? doc.splitTextToSize(rawLine, maxWidth) : ['']
...
export const buildChatPdfBlob = (personas: PrintablePersona[], notes = ''): Blob => {
