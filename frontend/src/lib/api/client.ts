...
// handling) without parsing the message string.
...
export async function streamChat(
    ...
    const errorName = (error: unknown): unknown =>
        error && typeof error === 'object' && 'name' in error ? (error as { name: unknown }).name : undefined
    const isTimeoutAbort = (error: unknown) =>
        errorName(error) === 'TimeoutError' || (errorName(error) === 'AbortError' && controller.signal.aborted)
    ...
    const parser = createParser({
        onEvent: (event: EventSourceMessage) => {
            ...
            onEvent({ type: (event.event ?? 'message') as StreamEvent['type'], data })
        },
    })
