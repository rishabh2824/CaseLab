...
// Converts a raw value to a string and escapes the three characters that would otherwise be interpreted as HTML markup
...
// turns null/undefined into empty string
const blank = (value) => (value == null ? '' : String(value))


// persona graph flattening
...
// 2-root + 1-referred pattern exported when the form has no personas yet.
function scaffoldPersonas() {
    ...
    return [
        { ...blankPersona(), referrals: [{ conditions: '', persona: blankPersona() }] },
        blankPersona(),
    ]
}


// All fields are a textarea so an LLM rewriting the file only ever edits text between tags
...
function personaCardMarkup(id, persona, isRoot) {
    ...
    ${fields({ field: 'availability_minutes', label: 'Available for (minutes)', hint: availabilityHint, value: persona?.availability_minutes })}
    ${fields({ field: 'known_facts', label: 'Persona Related Information', hint: 'Everything this persona knows and can draw on — background, facts, figures, opinions. Be specific; this grounds every reply they give.', value: persona?.known_facts, rows: 5 })}
    ${fields({ field: 'personality_traits', label: 'Personality Traits', hint: 'Tone, temperament, communication style, quirks.', value: persona?.personality_traits, rows: 3 })}
    ${fileShare(persona)}
  </article>`
}
...
function referralRowMarkup(edge, personaIds) {
    ...
    ${fields({ field: 'conditions', label: 'When', hint: 'The condition (in conversation) that makes the first persona introduce the second.', value: edge.conditions, rows: 2 })}
  </div>`
}
...
export function buildHTMLDoc({
    ...
    const effectivePersonas =
        Array.isArray(personas) && personas.length > 0 ? personas : scaffoldPersonas()
...
<div class="page">
  <article class="sheet">
    ...
    <section class="block">
      <div class="eyebrow"><span class="rule"></span><span>Case Setup</span><span class="rule"></span></div>
      <h2 class="section-title">Case Setup</h2>
      ${fields({ field: 'case_name', label: 'Case Name', value: caseName, required: true })}
      ${fields({ field: 'access_code', label: 'Access Code', hint: 'Students type this to start the simulation.', value: accessCode, required: true })}
      ${fields({ field: 'simulation_duration_minutes', label: 'Simulation Duration (minutes)', hint: 'Leave blank for unlimited.', value: simulationDurationMinutes })}
      ${fields({ field: 'initial_brief', label: 'Initial Brief', hint: 'What the student reads before the simulation begins — sets up the scenario and their objective.', value: initialBrief, rows: 5, required: true })}
      ${fields({ field: 'common_information', label: 'Case Background', hint: 'Shared context every persona in this case implicitly knows.', value: commonInformation, rows: 5 })}
    </section>

    <section class="block">
      <div class="eyebrow"><span class="rule"></span><span>Personas</span><span class="rule"></span></div>
      <h2 class="section-title">Personas</h2>
      <p class="section-hint">
        Every persona gets one card, whether available from the start
        (Root) or unlocked later via a referral (Referred) — switch that
        with the dropdown on each card.
      </p>
    ...
    <section class="block">
      <div class="eyebrow"><span class="rule"></span><span>Referrals</span><span class="rule"></span></div>
      <h2 class="section-title">Referrals</h2>
      <p class="section-hint">The first persona introduces the second once the condition below is met in conversation.</p>
...
<template id="persona-template">
  <article class="persona-card" data-persona-id="" data-persona-root="true">
    ...
    ${fields({ field: 'name', label: 'Name', value: '', required: true })}
    ${fields({ field: 'role', label: 'Role / Title', value: '', required: true })}
    ${fields({ field: 'availability_minutes', label: 'Available for (minutes)', hint: 'Blank = the whole simulation.', value: '' })}
    ${fields({ field: 'known_facts', label: 'Persona Related Information', hint: 'Everything this persona knows and can draw on — background, facts, figures, opinions. Be specific; this grounds every reply they give.', value: '', rows: 5 })}
    ${fields({ field: 'personality_traits', label: 'Personality Traits', hint: 'Tone, temperament, communication style, quirks.', value: '', rows: 3 })}
    ${fileShare(null)}
  </article>
</template>
...
<template id="referral-template">
  <div class="referral-row" data-referral="true">
    ...
    ${fields({ field: 'conditions', label: 'When', hint: 'The condition (in conversation) that makes the first persona introduce the second.', value: '', rows: 2 })}
  </div>
</template>
