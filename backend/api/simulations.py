...
from models.runtime import DeltaFrame, DoneFrame, ErrorFrame, TurnMeta
...
from services.simulation import state, stream
...
async def startSimulation(payload: StartSimulation) -> RunState:
    return await state.startSimulation(payload)


@router.get("/{run_id}")
async def getSimulationState(run_id: str) -> RunState:
    return await state.getSimulationState(run_id)


@router.get("/{run_id}/export")
async def exportSimulation(run_id: str) -> ExportResponse:
    return await state.exportSimulation(run_id)


@router.post("/{run_id}/message")
async def sendMessage(run_id: str, payload: SendMessage):
    return EventSourceResponse(stream.streamMessage(run_id, payload))
