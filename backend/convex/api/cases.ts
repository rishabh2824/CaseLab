...
// Read-only single-case fetch -- enough for DemoCaseView.svelte's hardcoded demo case.
// Deliberately public to any signed-in admin regardless of ownership: the demo case is a
// shared example, not access-controlled content. Distinct from getForEdit below, which
// enforces normal owner-or-collaborator access.
...
export const getForEdit = query({
	...
	handler: async (ctx, args) => {
		...
		const collaboratorRows = await ctx.db
			...
			.collect();
		return { ...c, collaboratorAdminIds: collaboratorRows.map((row) => row.adminId) };
	},
});
