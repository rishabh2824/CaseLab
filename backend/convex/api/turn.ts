...
export const start = mutation({
	...
	handler: async (ctx, args) => await startTurn(ctx, args.runId, args.personaId, args.message),
});
...
export const getTurnContext = internalQuery({
	...
	handler: async (ctx, args) => await getTurnContextService(ctx, args.runId, args.personaId),
});
...
export const runTurn = internalAction({
	args: { runId: v.id("runs"), personaId: v.string(), message: v.string(), streamId: v.id("streamingReplies") },
	handler: async (ctx, args) => await runTurnService(ctx, args.runId, args.personaId, args.message, args.streamId),
});
...
export const writeStreamingPreview = internalMutation({
	...
	handler: async (ctx, args) => await writeStreamingPreviewService(ctx, args.streamId, args.text),
});
...
export const getStreamingPreview = query({
	args: { runId: v.id("runs"), personaId: v.string() },
	handler: async (ctx, args) => await getStreamingPreviewService(ctx, args.runId, args.personaId),
});
...
export const applyBoundary = internalMutation({
	...
	handler: async (ctx, args) =>
		await applyBoundaryService(ctx, args.runId, args.personaId, args.label, args.personaName, args.streamId),
});
...
export const markStreamingError = internalMutation({
	...
	handler: async (ctx, args) => await markStreamingErrorService(ctx, args.streamId),
});
