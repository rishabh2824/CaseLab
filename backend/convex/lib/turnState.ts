...
export type Availability = { available: boolean; availableIn: number | null; expiresIn: number | null };
...
export function personaAvailability(
	...
	if (elapsed < availableAtMinutes) {
		return { available: false, availableIn: availableAtMinutes - elapsed, expiresIn: null };
	}
	if (availabilityDuration !== null) {
		const expiresAt = availableAtMinutes + availabilityDuration;
		if (elapsed > expiresAt) return { available: false, availableIn: null, expiresIn: 0 };
		return { available: true, availableIn: 0, expiresIn: Math.max(0, expiresAt - elapsed) };
	}
...
export type ChatStateMap = Record<string, { warningCount: number; ended: boolean; endReason?: string }>;
export type ChatStateOut = { ended: boolean; endReason: string | null; warningCount: number };
...
export function getChatState(personaChatState: ChatStateMap, personaId: string): ChatStateOut {
	const state = personaChatState[personaId];
	return state
		? { ended: state.ended, endReason: state.endReason ?? null, warningCount: state.warningCount }
