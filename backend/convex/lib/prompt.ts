...
export type CandidateReferral = { handle: string; name: string; role: string; conditionTrigger: string };
export type CandidateFile = { handle: string; name: string; perceivedContents: string | null; shareConditions: string | null };
...
export function systemPrompt(
	...
	if (candidateReferrals.length > 0) {
		const options = candidateReferrals
			.map((c) => `${c.handle}: ${c.name} (${c.role}) — unlock condition: ${c.conditionTrigger}`)
			.join("; ");
		referralLines.push(
	...
	if (candidateReferrals.length > 0 && knownFacts !== "None") {
		for (const c of candidateReferrals) {
			...
			knownFacts = knownFacts.replace(new RegExp(`\\b${escapeRegExp(c.name)}\\b`, "g"), "[undisclosed contact]");
		}
	}
...
export function parseReply(raw: string | null | undefined): Record<string, unknown> | null {
	const text = (raw ?? "").trim();
	if (!text) return null;
	let data: unknown;
	try {
