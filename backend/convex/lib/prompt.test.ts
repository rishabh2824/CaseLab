...
describe("systemPrompt", () => {
	...
	it("lists referral and file candidates with their handles and conditions", () => {
		const prompt = systemPrompt(
			...
			COMMON_INFO,
			personaDetail(),
			[{ handle: "R1", name: "Bob", role: "COO", conditionTrigger: "the user asks for the COO" }],
			[{ handle: "F1", name: "Budget.pdf", perceivedContents: "last quarter's numbers", shareConditions: "the user asks about the budget" }],
		);
		...
		expect(prompt).toContain("F1: Budget.pdf (what you believe it contains: last quarter's numbers)");
		expect(prompt).toContain("sharing condition: the user asks about the budget");
	});
	...
	it("omits the parenthetical when a file has no perceived contents", () => {
		const prompt = systemPrompt(BRIEF, COMMON_INFO, personaDetail(), [], [
			{ handle: "F2", name: "Empty.pdf", perceivedContents: "", shareConditions: "the user asks" },
		]);
	...
	it("adds the never-introduce-anyone-unlisted sentence whenever there are referral candidates", () => {
		const prompt = systemPrompt(BRIEF, COMMON_INFO, personaDetail(), [{ handle: "R1", name: "Karen", role: "Support Lead", conditionTrigger: "cond" }], []);
		expect(prompt).toContain("Never introduce, mention, hint at, or offer to connect the user");
	});
	...
	describe("redaction (security-critical)", () => {
		it("redacts a candidate referral's name out of known facts, regardless of its condition", () => {
			const prompt = systemPrompt(
				BRIEF,
				COMMON_INFO,
				personaDetail({ knownFacts: "Karen handles all complaint escalations." }),
				[{ handle: "R1", name: "Karen", role: "Support Lead", conditionTrigger: "the user asks for support" }],
				[],
			);
		...
		it("only matches whole words, not substrings of a longer name", () => {
			...
			const prompt = systemPrompt(
				...
				[{ handle: "R1", name: "Ann", role: "Analyst", conditionTrigger: "cond" }],
				[],
			);
		...
		it("escapes regex metacharacters in the candidate name instead of crashing or over-matching", () => {
			const prompt = systemPrompt(
				...
				[{ handle: "R1", name: "J. Smith", role: "Approver", conditionTrigger: "cond" }],
				[],
			);
			expect(prompt).not.toContain("Reach out to J. Smith for approvals.");
			expect(prompt).toContain("Reach out to [undisclosed contact] for approvals.");
		});
		...
		it("skips a blank/whitespace candidate name without erroring, and still redacts a real one", () => {
			const prompt = systemPrompt(
				...
				[
					...
					{ handle: "R2", name: "Karen", role: "Support Lead", conditionTrigger: "cond" },
				],
				[],
			);
			expect(prompt).toContain("[undisclosed contact]");
...
describe("parseReply", () => {
	it("parses a clean JSON object", () => {
		...
		expect(parseReply(raw)).toEqual({ reply: "Hi there.", introduce: ["R1"], send_files: [] });
	});
