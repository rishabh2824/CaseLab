...
import { guardNoLiveRuns } from "../lib/guardNoLiveRuns";
import type { FileRefPayload, PersonaPayload, ReferralEdgePayload } from "../models/cases";
import { resolveFileRefs, syncCaseFiles, type ResolvedFileRef } from "./files";

const ACCESS_CODE_CONFLICT = "An access code with this value already exists on another case.";
...
export async function listCases(ctx: QueryCtx, admin: Doc<"admins">): Promise<Doc<"cases">[]> {
	if (admin.role === "super") {
	...
	const collaboratingRows = await ctx.db
		...
		.collect();
	const collaboratingCases = await Promise.all(collaboratingRows.map((row) => ctx.db.get(row.caseId)));
...
export async function deleteCase(ctx: MutationCtx, caseId: Id<"cases">, admin: Doc<"admins">): Promise<void> {
	const c = await ctx.db.get(caseId);
	if (!c) throw new Error("Case not found.");
	await requireCaseAccess(ctx, c, admin);
	await guardNoLiveRuns(ctx, caseId);

	await syncCaseFiles(ctx, caseId, new Set());

	const collaboratorRows = await ctx.db.query("collaborators").withIndex("by_case", (q) => q.eq("caseId", caseId)).collect();
...
// save is about to write must have unique persona ids, roots/referral endpoints that all
// point at real personas, and no referral cycle.
export function validateGraph(personas: PersonaPayload[], referrals: ReferralEdgePayload[], roots: string[]): void {
	const personaIds = personas.map((p) => p.id);
	const seen = new Set<string>();
	const duplicates = new Set<string>();
	for (const id of personaIds) (seen.has(id) ? duplicates : seen).add(id);
	if (duplicates.size > 0) {
		throw new Error(`Duplicate persona id(s): ${[...duplicates].sort().join(", ")}`);
	}

	const validIds = seen;
	for (const rootId of roots) {
		if (!validIds.has(rootId)) throw new Error(`Unknown root persona id: ${rootId}`);
	}
	for (const referral of referrals) {
		if (!validIds.has(referral.from_id)) throw new Error(`Unknown referral from_id: ${referral.from_id}`);
		if (!validIds.has(referral.to_id)) throw new Error(`Unknown referral to_id: ${referral.to_id}`);
	}
	...
	const IN_PROGRESS = 1;
	const DONE = 2;
	const state = new Map<string, number>(personaIds.map((id) => [id, UNVISITED]));

	function visit(node: string): string | null {
		state.set(node, IN_PROGRESS);
		for (const neighbor of adjacency.get(node) ?? []) {
			if (state.get(neighbor) === IN_PROGRESS) return neighbor;
	...
	for (const personaId of personaIds) {
		if (state.get(personaId) === UNVISITED) {
			const cycleNode = visit(personaId);
			if (cycleNode !== null) throw new Error(`Referral cycle detected involving persona id: ${cycleNode}`);
		}
	}
}

function toFileRefInput(ref: FileRefPayload | undefined) {
	return ref ? { objectKey: ref.object_key, fileName: ref.file_name, contentType: ref.content_type ?? undefined } : null;
}
...
export async function buildStructure(
	...
	const outReferrals = referrals.map((r) => ({ from_id: r.from_id, to_id: r.to_id, conditions: r.conditions }));

	return { structure: { personas: outPersonas, referrals: outReferrals, roots: [...roots] }, fileIds };
}
...
async function resolveCollaboratorIds(
	...
	if (missing.length > 0) throw new Error(`Unknown admin id(s): ${missing.join(", ")}`);
...
async function replaceCollaborators(
	...
	for (const adminId of collaboratorIds) {
		await ctx.db.insert("collaborators", { caseId, adminId, addedAt: Date.now() });
	}
}

// Mirrors backend/services/cases.py's createCase.
export async function createCase(ctx: MutationCtx, payload: CasePayload, admin: Doc<"admins">): Promise<Id<"cases">> {
	const accessCode = await validateAccessCode(ctx, payload.accessCode);
	const collaboratorIds = await resolveCollaboratorIds(ctx, payload.collaboratorAdminIds, admin._id);
	const { structure, fileIds } = await buildStructure(ctx, payload.personas, payload.referrals, payload.roots);
...
export async function updateCase(
	...
	const collaboratorIds = await resolveCollaboratorIds(ctx, payload.collaboratorAdminIds, c.ownerAdminId);
	const { structure, fileIds } = await buildStructure(ctx, payload.personas, payload.referrals, payload.roots);
