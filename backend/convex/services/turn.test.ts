...
import { caseStructure, personaPayload, referralEdge, fileEntry } from "../testFactories";
import { startSimulation, getSimulationState, getPersonaHistory } from "./simulations";
...
function sseResponse(envelope: string): Response {
	return new Response(`data: ${JSON.stringify({ choices: [{ delta: { content: envelope } }] })}\n\ndata: [DONE]\n\n`, {
		status: 200,
	});
}

function resolveMaybeFn<T>(value: T | ((message: string, conversation: string) => T), message: string, conversation: string): T {
	return typeof value === "function" ? (value as (m: string, h: string) => T)(message, conversation) : value;
}
...
function stubLlm(options: LlmStubOptions = {}) {
	const { harassment = "normal", replyText = "OK", introduce = [], sendFiles = [] } = options;
	const calls: { kind: string; body: any }[] = [];
	const fetchMock = vi.fn(async (_url: string, init: RequestInit) => {
		const body = JSON.parse(init.body as string);
		calls.push({ kind: body.stream ? "reply" : "classify", body });
		if (body.stream) {
			const envelope = JSON.stringify({ reply: replyText, introduce, send_files: sendFiles });
...
async function startRun(t: ReturnType<typeof newTestConvex>, structure: unknown, accessCode = "sterling") {
	const ownerAdminId = await t.run((ctx) => ctx.db.insert("admins", { email: `o-${Math.random()}@test.caselab.invalid`, role: "admin" }));
	await t.run((ctx) => ctx.db.insert("cases", { name: "Case", brief: "Brief", accessCode, ownerAdminId, structure }));
	return await t.run((ctx) => startSimulation(ctx, accessCode));
}

async function send(t: ReturnType<typeof newTestConvex>, runId: Id<"runs">, personaId: string, message: string) {
...
describe("startTurn validation", () => {
	it("rejects an empty/whitespace message", async () => {
		...
		await expect(t.run((ctx) => startTurn(ctx, state.run_id, "A", "   "))).rejects.toThrow();
	});
	...
	it("rejects a message over the word limit", async () => {
		...
		await expect(t.run((ctx) => startTurn(ctx, state.run_id, "A", tooLong))).rejects.toThrow(/too long/);
	});

	it("rejects an unknown persona id", async () => {
		const t = newTestConvex();
		const state = await startRun(t, caseStructure());
		await expect(t.run((ctx) => startTurn(ctx, state.run_id, "does-not-exist", "hi"))).rejects.toThrow("Persona not found.");
	});
	...
	it("rejects a persona that exists in the graph but hasn't been unlocked yet", async () => {
		...
		await expect(t.run((ctx) => startTurn(ctx, state.run_id, "B", "hi"))).rejects.toThrow("Persona is not available yet.");
	});
	...
	it("rejects a message once the simulation's duration has elapsed", async () => {
		...
		await t.run((ctx) => ctx.db.patch(state.run_id, { startTime: run.startTime - 15 * 60_000 }));
		await expect(t.run((ctx) => startTurn(ctx, state.run_id, "A", "hi"))).rejects.toThrow("This simulation has ended.");
	});

	it("rejects a message once the persona's own availability window has expired", async () => {
		const t = newTestConvex();
		const structure = caseStructure({ personas: [personaPayload("A", { availability_minutes: 5 })] });
		const state = await startRun(t, structure);
		await t.run((ctx) => ctx.db.patch(state.run_id, { startTime: Date.now() - 10 * 60_000 }));
		await expect(t.run((ctx) => startTurn(ctx, state.run_id, "A", "hi"))).rejects.toThrow("Persona is not available yet.");
	});
});
...
describe("concurrency guard (claimStreamingSlot)", () => {
	it("rejects a second turn on the same persona while one is already in flight", async () => {
		...
		await expect(t.run((ctx) => startTurn(ctx, state.run_id, "A", "second message"))).rejects.toThrow(
...
describe("normal turn happy path", () => {
	it("persists the reply and clears the streaming preview", async () => {
		...
		const history = await t.run((ctx) => getPersonaHistory(ctx, state.run_id, "A"));
		expect(history.map((m) => m.content)).toEqual(["What vendor do we use?", "Our vendor is Acme."]);
	});

	it("strips a leading speaker tag before storing the reply", async () => {
		const t = newTestConvex();
		const state = await startRun(t, caseStructure({ personas: [personaPayload("A", { name: "Mary" })] }));
	...
	it("bounds what reaches the LLM to RECENT_HISTORY_LIMIT but persists the full history", async () => {
		...
		expect(sentMessages[sentMessages.length - 1]).toEqual({ role: "user", content: "turn-7" });

		const fullHistory = await t.run((ctx) => getPersonaHistory(ctx, state.run_id, "A"));
	...
	it("preserves the user's message and marks the stream errored when the LLM call fails", async () => {
		const t = newTestConvex();
		const state = await startRun(t, caseStructure());
		vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("upstream blew up")));
...
describe("harassment/boundary escalation", () => {
	function twoRoots() {
		return caseStructure({ personas: [personaPayload("A"), personaPayload("B")], roots: ["A", "B"] });
	}
	...
	it("still issues the reply call concurrently even when the message ends up flagged", async () => {
		...
		const run = (await t.run((ctx) => ctx.db.get(state.run_id)))!;
		expect(run.personaChatState["A"]).toMatchObject({ warningCount: 1, ended: false });
	});

	it("never reveals the discarded reply to the client and never persists it", async () => {
		const t = newTestConvex();
		const state = await startRun(t, twoRoots());
		stubLlm({ harassment: "nonsense", replyText: "this in-character reply must never be shown" });

		await send(t, state.run_id, "A", "bad message");

		// Nothing was ever written to the live preview a client subscribes to mid-stream.
		const preview = await t.run((ctx) => getStreamingPreview(ctx, state.run_id, "A"));
		expect(preview).toEqual({ text: "", status: "done" });

		const history = await t.run((ctx) => getPersonaHistory(ctx, state.run_id, "A"));
		const assistantReply = history.find((m) => m.role === "assistant")!;
		expect(assistantReply.content).not.toContain("this in-character reply must never be shown");
	...
	it("ends the chat once NONSENSE_THRESHOLD is reached", async () => {
		...
		for (let i = 0; i < NONSENSE_THRESHOLD; i++) await send(t, state.run_id, "A", "bad message");

		const run = (await t.run((ctx) => ctx.db.get(state.run_id)))!;
		expect(run.personaChatState["A"]).toMatchObject({ warningCount: NONSENSE_THRESHOLD, ended: true, endReason: "nonsense" });
		const history = await t.run((ctx) => getPersonaHistory(ctx, state.run_id, "A"));
		expect(history[history.length - 1]?.content).toContain("ending this conversation");
	});
	...
	it("rejects a message to an already-ended chat, even a clean one", async () => {
		...
		await expect(t.run((ctx) => startTurn(ctx, state.run_id, "A", "sorry, can we continue?"))).rejects.toThrow(
	...
	it("ending one persona's chat does not end another persona's", async () => {
		...
		const run = (await t.run((ctx) => ctx.db.get(state.run_id)))!;
		expect(run.personaChatState["B"]).toMatchObject({ warningCount: 1, ended: false });
	});
...
describe("referrals", () => {
	function caseWithOneReferral() {
		return caseStructure({
			personas: [personaPayload("A", { name: "Alice" }), personaPayload("B", { name: "Bob", role: "Analyst" })],
			referrals: [referralEdge("A", "B", "the user explicitly asks to speak with Bob")],
			roots: ["A"],
		});
	}
	...
	it("offers a candidate referral with its condition in the prompt, and introducing it unlocks the contact", async () => {
		...
		const { calls } = stubLlm({ replyText: "I'll connect you with Bob.", introduce: ["R1"] });

		await send(t, state.run_id, "A", "Can I talk to someone else?");

		const replyCall = calls.find((c) => c.kind === "reply")!;
		expect(replyCall.body.messages[0].content).toContain("R1: Bob (Analyst)");
		expect(replyCall.body.messages[0].content).toContain("unlock condition: the user explicitly asks to speak with Bob");
	...
	it("redacts a pending referral's name from the prompt, even after it was mentioned in a reply", async () => {
		...
		const structure = caseStructure({
			personas: [
				personaPayload("A", { name: "Alice", known_facts: "Bob is the analyst who audited the budget." }),
		...
		const replyCall = calls.find((c) => c.kind === "reply")!;
		expect(replyCall.body.messages[0].content).toContain("[undisclosed contact]");
		const run = (await t.run((ctx) => ctx.db.get(state.run_id)))!;
	...
	it("re-unlocking an already-unlocked persona (a retried applyDecisions call) is a no-op", async () => {
		...
		const streamId = await t.run((ctx) =>
			ctx.db.insert("streamingReplies", { runId: state.run_id, personaKey: "A", text: "", status: "streaming" }),
		);
		await t.run((ctx) => applyDecisions(ctx, state.run_id, "A", "Sure, meet Bob.", [{ referredPersonaId: "B" }], [], streamId));
		const firstUnlockedAt = (await t.run((ctx) => ctx.db.get(state.run_id)))!.unlockedAt["B"];

		// Time passes before the (simulated) retry.
		const before = (await t.run((ctx) => ctx.db.get(state.run_id)))!;
		await t.run((ctx) => ctx.db.patch(state.run_id, { startTime: before.startTime - 5 * 60_000 }));
		await t.run((ctx) => applyDecisions(ctx, state.run_id, "A", "Sure, meet Bob again.", [{ referredPersonaId: "B" }], [], streamId));
	...
	it("stops offering a persona referred by two parents to the parent who didn't unlock it", async () => {
		const t = newTestConvex();
		const structure = caseStructure({
			personas: [personaPayload("A", { name: "Alice" }), personaPayload("D", { name: "Dana" }), personaPayload("B", { name: "Bob", role: "Analyst" })],
			referrals: [referralEdge("A", "B", "A asks about Bob"), referralEdge("D", "B", "D asks about Bob")],
			roots: ["A", "D"],
		});
		const state = await startRun(t, structure);
		stubLlm({ replyText: "Meet Bob.", introduce: ["R1"] });
		...
		expect(replyCall.body.messages[0].content).toContain("You have no one to introduce this turn.");
	});

	it("numbers handles stably and one-indexed across multiple pending referrals", async () => {
		const t = newTestConvex();
		const structure = caseStructure({
			personas: [personaPayload("A", { name: "Alice" }), personaPayload("B", { name: "Bob" }), personaPayload("C", { name: "Carl" }), personaPayload("E", { name: "Erin" })],
			referrals: [referralEdge("A", "B", "cond 1"), referralEdge("A", "C", "cond 2"), referralEdge("A", "E", "cond 3")],
			roots: ["A"],
		});
		const state = await startRun(t, structure);
		const { calls } = stubLlm({ replyText: "hi" });
...
describe("files", () => {
	...
	async function startRunWithOneFile(t: ReturnType<typeof newTestConvex>) {
		const fileId = await t.run((ctx) => ctx.db.insert("files", { objectKey: "cases/1/budget.pdf", name: "budget.pdf", contentType: "application/pdf" }));
		const structure = caseStructure({
			personas: [
				personaPayload("A", {
					files: [fileEntry({ file_id: fileId, object_key: "cases/1/budget.pdf", file_name: "budget.pdf", share_conditions: "the user asks about the budget", perceived_contents: "Q3 numbers" })],
				}),
			],
		});
		const state = await startRun(t, structure);
	...
	it("offers a candidate file with its condition in the prompt, and sending it shares a stable url", async () => {
		...
		const { calls } = stubLlm({ replyText: "Here's the budget.", sendFiles: ["F1"] });

		await send(t, state.run_id, "A", "Can I see the budget?");

		const replyCall = calls.find((c) => c.kind === "reply")!;
		expect(replyCall.body.messages[0].content).toContain("F1: budget.pdf (what you believe it contains: Q3 numbers)");
		expect(replyCall.body.messages[0].content).toContain("sharing condition: the user asks about the budget");

		const run = (await t.run((ctx) => ctx.db.get(state.run_id)))!;
		expect(Object.keys(run.sharedFiles)).toContain(fileId);

		const live = await t.run((ctx) => getSimulationState(ctx, state.run_id));
		expect(live.shared_files[0].url).toBe("https://example.convex.site/files/cases/1/budget.pdf");
	});
	...
	it("never shares a file with a blank share condition, and never even offers it as a candidate", async () => {
		...
		const structure = caseStructure({
			personas: [
				personaPayload("A", {
					files: [fileEntry({ file_id: fileId, object_key: "cases/1/budget.pdf", file_name: "budget.pdf", share_conditions: "", perceived_contents: "Q3 numbers" })],
				}),
			],
		});
		const state = await startRun(t, structure);
		...
		const replyCall = calls.find((c) => c.kind === "reply")!;
		expect(replyCall.body.messages[0].content).toContain("You have no file to send this turn.");
	...
	it("never offers a file with no file_id, and withholds it from the prompt", async () => {
		const t = newTestConvex();
		const structure = caseStructure({ personas: [personaPayload("A", { files: [fileEntry({ file_id: null })] })] });
		const state = await startRun(t, structure);
	...
	it("does not re-offer or re-share an already-shared file", async () => {
		...
		const { calls: secondCalls } = stubLlm({ replyText: "Anything else?", sendFiles: ["F1"] });
		await send(t, state.run_id, "A", "Anything else about the budget?");
		const secondReplyCall = secondCalls.find((c) => c.kind === "reply")!;
		expect(secondReplyCall.body.messages[0].content).toContain("You have no file to send this turn.");
		expect(secondReplyCall.body.messages[0].content).not.toContain("budget.pdf");
	...
	it("ignores an unknown file handle from the model", async () => {
		const t = newTestConvex();
		const state = await startRun(t, caseStructure({ personas: [personaPayload("A", { files: [] })] }));
