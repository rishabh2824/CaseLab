...
import { flattenPersonas, graphPersonaById, graphPersonas, graphReferrals, graphRootPersonas } from "./simulationReads";
...
describe("flattenPersonas", () => {
	...
	it("sorts root rows by name", () => {
		const structure = caseStructure({
			personas: [personaPayload("A", { name: "Zed" }), personaPayload("B", { name: "Alice" })],
			roots: ["A", "B"],
		});
		const graph = flattenPersonas(structure);
		expect(graph.roots.map((id) => graph.personas.get(id)!.name)).toEqual(["Alice", "Zed"]);
	});
	...
	it("turns every referral into an edge, with a null condition becoming an empty string", () => {
		...
		expect(graph.referrals[0]).toEqual({ parentPersonaId: "A", referredPersonaId: "B", conditionTrigger: "" });
	});
	...
	it("stores a persona referred by two parents once, not once per edge", () => {
		...
		const graph = flattenPersonas(structure);
		const targets = new Set(graph.referrals.map((e) => `${e.parentPersonaId}->${e.referredPersonaId}`));
...
function buildGraph(): PersonaGraph {
	const structure = caseStructure({
		personas: [
			personaPayload("A"),
			personaPayload("B"),
			personaPayload("C", { profile_photo: { file_id: null, object_key: "cases/1/c.png", file_name: "c.png", content_type: "image/png" } }),
			personaPayload("D"),
		],
		referrals: [referralEdge("A", "C"), referralEdge("B", "C"), referralEdge("A", "D")],
		roots: ["A", "B"],
	});
...
describe("graphReferrals", () => {
	it("filters edges by parent, returning raw (unhydrated) personas", () => {
		...
		const referredIds = new Set(graphReferrals(graph, "A").map((e) => e.referredPersonaId));
		expect(referredIds).toEqual(new Set(["C", "D"]));

		const c = graphPersonaById(graph, "C")!;
		expect(c.profilePhotoUrl).toBeNull();
		expect(c.profilePhoto?.object_key).toBe("cases/1/c.png");
	});
});
...
describe("graphPersonaById", () => {
	...
	it("finds a referred persona", () => {
		const graph = flattenPersonas(
			caseStructure({ personas: [personaPayload("A"), personaPayload("B")], referrals: [referralEdge("A", "B")] }),
		);
