...
async function seedCase(
	...
	const ownerAdminId = await t.run((ctx) => ctx.db.insert("admins", { email: `owner-${Math.random()}@test.caselab.invalid`, role: "admin" }));
...
describe("startSimulation", () => {
	it("rejects a blank access code before any case lookup", async () => {
		...
		await expect(t.run((ctx) => startSimulation(ctx, "   "))).rejects.toThrow("Access code is required.");
	});

	it("rejects an unknown access code", async () => {
		const t = newTestConvex();
		await seedCase(t, { accessCode: "sterling" });
		await expect(t.run((ctx) => startSimulation(ctx, "not-a-real-code"))).rejects.toThrow("Invalid access code.");
	});

	it("rejects a case with no root personas", async () => {
		const t = newTestConvex();
		await seedCase(t, { accessCode: "empty", structure: caseStructure({ roots: [] }) });
		await expect(t.run((ctx) => startSimulation(ctx, "empty"))).rejects.toThrow("This case has no personas configured.");
	});
	...
	it("returns a trimmed case summary and one contact per root", async () => {
		...
		await seedCase(t, {
			...
			brief: "Do the thing.",
			duration: 30,
			structure: caseStructure({ personas: [personaPayload("A", { name: "Alice" }), personaPayload("B", { name: "Bob" })], roots: ["A", "B"] }),
		});

		const state = await t.run((ctx) => startSimulation(ctx, "acme"));

		expect(state.case).toEqual({ id: state.case.id, case_name: "Acme Case", brief: "Do the thing.", simulation_duration: 30 });
		expect(state.contacts).toHaveLength(2);
		expect(new Set(state.contacts.map((c) => c.id))).toEqual(new Set(["A", "B"]));
	...
	it("never leaks a persona's secret fields into a contact", async () => {
		...
		await seedCase(t, {
			...
			structure: caseStructure({
				personas: [personaPayload("A", { known_facts: secretFact, personality_traits: "Blunt, impatient." })],
			}),
		});
	...
	it("does not default to a root whose availability window has already closed", async () => {
		...
		await seedCase(t, {
			accessCode: "window",
			structure: caseStructure({
				personas: [personaPayload("A", { name: "Alice", availability_minutes: 0 }), personaPayload("B", { name: "Bob" })],
				roots: ["A", "B"],
			}),
		});
	...
	it("resolves a profile photo to the stable /files/:objectKey path", async () => {
		const t = newTestConvex();
		await seedCase(t, {
			accessCode: "photo",
			structure: caseStructure({
				personas: [
					personaPayload("A", {
						profile_photo: { file_id: "photo-1", object_key: "cases/1/alice.png", file_name: "alice.png", content_type: "image/png" },
					}),
				],
			}),
		});
		...
		expect(state.contacts[0].profile_photo?.url).toBe("https://example.convex.site/files/cases/1/alice.png");
	});
...
describe("getSimulationState", () => {
	it("throws for an unknown run", async () => {
		const t = newTestConvex();
		await expect(t.run((ctx) => getSimulationState(ctx, "k17unknown0000000000000" as Id<"runs">))).rejects.toThrow(
			"Run not found.",
		);
	});
	...
	it("throws for an expired run without deleting it -- cleanup is the scheduled job's responsibility, not a read's", async () => {
		...
		await t.run((ctx) => ctx.db.patch(state.run_id, { expiresAt: Date.now() - 1_000 }));

		await expect(t.run((ctx) => getSimulationState(ctx, state.run_id))).rejects.toThrow("Run expired.");
	...
	it("includes an unlocked referred persona, available from its unlock minute", async () => {
		...
		await t.run((ctx) => ctx.db.patch(state.run_id, { unlockedReferredIds: ["B"], unlockedAt: { B: 7 } }));
...
describe("exportSimulation", () => {
	it("orders roots then referred personas by unlock time, not referral-authoring order", async () => {
		const t = newTestConvex();
		const structure = caseStructure({
			personas: [personaPayload("A", { name: "Alice" }), personaPayload("B", { name: "Bob" }), personaPayload("C", { name: "Carl" }), personaPayload("D", { name: "Dana" })],
			referrals: [referralEdge("A", "C", "unlock Carl"), referralEdge("B", "D", "unlock Dana")],
			roots: ["A", "B"],
		});
		const state = await startRun(t, structure);
		// D unlocked before C, despite C's referral being authored first -- export order must
		// follow unlock time, not authoring order.
		await t.run((ctx) => ctx.db.patch(state.run_id, { unlockedReferredIds: ["C", "D"], unlockedAt: { D: 1, C: 5 } }));
		await t.run((ctx) => ctx.db.insert("runMessages", { runId: state.run_id, personaKey: "C", role: "user", content: "hi Carl" }));
		await t.run((ctx) => ctx.db.insert("runMessages", { runId: state.run_id, personaKey: "C", role: "assistant", content: "Hello, I'm Carl." }));
...
describe("deleteRunCascade", () => {
	it("deletes the run's messages and streaming-preview rows along with the run itself", async () => {
		const t = newTestConvex();
		const state = await startRun(t, caseStructure());
		await t.run((ctx) => ctx.db.insert("runMessages", { runId: state.run_id, personaKey: "A", role: "user", content: "hi" }));
		await t.run((ctx) => ctx.db.insert("streamingReplies", { runId: state.run_id, personaKey: "A", text: "", status: "streaming" }));

		await t.run((ctx) => deleteRunCascade(ctx, state.run_id));

		expect(await t.run((ctx) => ctx.db.get(state.run_id))).toBeNull();
		const messages = await t.run((ctx) => ctx.db.query("runMessages").withIndex("by_run_persona", (q) => q.eq("runId", state.run_id)).collect());
		const streaming = await t.run((ctx) => ctx.db.query("streamingReplies").withIndex("by_run_persona", (q) => q.eq("runId", state.run_id)).collect());
...
describe("deleteExpiredRuns", () => {
	it("removes only expired rows and returns the count deleted", async () => {
		...
		await t.run((ctx) => ctx.db.patch(stale.run_id, { expiresAt: Date.now() - 1_000 }));
