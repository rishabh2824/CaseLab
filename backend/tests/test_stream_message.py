...
class TestBoundaryPassthrough:
    async def test_yields_precomputed_meta_delta_done_without_generating(self):
        ...
        raw_events = [event async for event in service.streamMessage(boundary_prepared)]
        assert events(raw_events) == ["meta", "delta", "done"]
    ...
    async def test_empty_boundary_reply_skips_delta(self):
        ...
        raw_events = [event async for event in service.streamMessage(boundary_prepared)]
        assert events(raw_events) == ["meta", "done"]
...
class TestNormalReplyHappyPath:
    ...
    async def test_fallback_single_delta_when_extractor_never_finds_reply(self, monkeypatch):
        ...
        monkeypatch.setattr(service, "ReplyExtractor", BlindExtractor)
        monkeypatch.setattr(
            service, "personaReplyStream",
        ...
        assert events(raw_events) == ["delta", "meta", "done"]
        import json
        assert json.loads(raw_events[0]["data"])["text"] == "Recovered"
