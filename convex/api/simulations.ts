...
// Public and unauthenticated -- a student is gated only by knowing a valid access code, not
// by signing in.
...
// No per-persona histories -- see RunStateOut's own comment (services/simulations.ts) for
// why. getPersonaHistory (below) is the scoped replacement.
...
// Named `exportRun`, not `export` -- the latter is a reserved word and can't be a binding
// name.
...
// precise per-run deletion instead of a full-table sweep.
